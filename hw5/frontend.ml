...
let oat_alloc_struct ct (id : Ast.id) : Ll.ty * operand * stream =
  ...
  let size = Int64.of_int (8 * length) in
...
let rec cmp_exp (tc : TypeCtxt.t) (c : Ctxt.t) (exp : Ast.exp node) :
  ...
  *)
  (*
  var a = new int[e1] {id -> e2};
  var temparr = a 
   for(var id = 0; id < length(temparr) ; id = id + 1;) {
      temparr[x] = e2;
   } *)
  | Ast.NewArr (elt_ty, e1, id, e2) ->
      ...
      let temparr_code =
        [ E (temparr_uid, Alloca arr_ty) ]
        >@ [ I ("", Store (arr_ty, arr_op, Id temparr_uid)) ]
      in
      let cond =
        Some
          ( no_loc
          @@ Bop (Lt, no_loc (Id id), no_loc (Length (no_loc (Id temparr)))) )
      in
  ...
  | Ast.CStruct (id, l) ->
      ...
      let stream =
        List.fold_left
          (fun list (vn, e) ->
            (* { i64, [3 x i64] }* *)
            let lookty_as, index = TypeCtxt.lookup_field_name id vn tc in
            let lookty = cmp_ty tc lookty_as in
            let vop, vcode = cmp_exp_as tc c e lookty in

            let field = gensym "fieldtmp" in
            let gepins =
              I (field, Gep (struct_ty, struct_op, [ Const 0L; Const index ]))
            in
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c : Ctxt.t) (e : exp node) :
  ...
  *)
  | Ast.Proj (e, i) ->
      ...
      let sname : string =
        ...
        | _ -> failwith "this is not good struct"
      in

      let astty = TypeCtxt.lookup_field sname i tc in
      let index = TypeCtxt.index_of_field sname i tc in
      let ans_id = gensym "proj" in
      let ty = cmp_ty tc astty in
      let gepins =
        I
          ( ans_id,
            Gep (struct_ty, struct_op, [ Const 0L; Const (Int64.of_int index) ])
          )
      in
      (ty, Ll.Id ans_id, struct_code >@ [ gepins ])
  ...
  *)
  | Ast.Index (e, i) ->
      ...
      in
      let ptr_id, tmp_id, cast_id =
        (gensym "index_ptr", gensym "tmp", gensym "cast")
      in
      (* can't use cmp_exp_as because i need the old type for gep*)
      let cast_ins = I (cast_id, Bitcast (arr_ty, arr_op, Ptr I64)) in
      let call_bound_check =
        I
          ( "",
            Ll.Call
              ( Void,
                Gid "oat_assert_array_length",
                [ (Ptr I64, Ll.Id cast_id); (I64, ind_op) ] ) )
      in

      ( ans_ty,
        Id ptr_id,
        arr_code >@ ind_code >:: cast_ins >:: call_bound_check
        >@ lift
             [
               ( ptr_id,
                 Gep
...
and cmp_stmt (tc : TypeCtxt.t) (c : Ctxt.t) (rt : Ll.ty) (stmt : Ast.stmt node)
  ...
  *)
  | Ast.Cast (rtyp, id, exp, notnull, null) ->
      let ll_ty_noptr = cmp_rty tc rtyp in
      let ll_ty = Ptr ll_ty_noptr in

      let e_op, e_code = cmp_exp_as tc c exp ll_ty in

      let guard = gensym "castguard" in

      let lt, le, lm = (gensym "then", gensym "else", gensym "merge") in

      let name = gensym id in

      let cnew = Ctxt.add c id (Ptr ll_ty, Ll.Id name) in
      let notnull_code = cmp_block tc cnew rt notnull in
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e : Ast.exp node) :
  ...
  | CStruct (id, cs) ->
      ...
      let elts, gs =
        List.fold_right
          (fun (_, e) (elts, gs) ->
            let gd, gs' = cmp_gexp c tc e in
            (gd :: elts, gs' @ gs))
          sorted_cs ([], [])
      in

      let gid = gensym "global_cstruct" in
