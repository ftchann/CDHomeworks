...
let rec lookup (x:string) (c:ctxt) : int64 =
  begin match c with
    | [] -> raise Not_found
    | (y, v)::ys -> if x = y then v else lookup x ys
  end
...
let rec optimize (e:exp) : exp =
  begin match e with
    | Add (a, b) ->  begin match optimize a, optimize b with
                       | (Const a, Const b) -> Const (Int64.add a b)  
                       | (a, Const 0L) -> a  
                       | (Const 0L, b) -> b
                       | (a, b) when a = Neg b || b = Neg a -> Const(0L) 
                       | (Neg a, Neg b) -> optimize (Neg(Add(a,b)))
                       | (a, b) -> Add(a,b)
                     end
    | Mult (a, b) -> begin match optimize a, optimize b with
                       | (Const 0L, b) -> Const 0L
                       | (a, Const 0L) -> Const 0L
                       | (a, Const 1L) -> a
                       | (a, Const -1L) -> optimize (Neg a)
                       | (Const 1L, b) -> b
                       | (Const -1L, b) -> optimize (Neg b)
                       | (Neg a, Neg b) -> (optimize (Mult(a,b)))
                       | (Const a, Neg b) ->  (optimize (Mult(Const (Int64.neg a), b)))
                       | (Neg a, Const b) -> (optimize (Mult(a, Const (Int64.neg b))))
                       | (a, Neg b) -> optimize (Neg(Mult(a,b)))
                       | (Neg a, b) -> optimize (Neg(Mult(a,b)))
                       | (a, b) -> Mult(a,b)
                     end
    | Neg a       -> begin match optimize a with
